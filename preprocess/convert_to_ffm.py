...
if __name__ == '__main__':
    ...
    out_train_path = os.path.join(curr_dir, 'output', 'trainffm_' + args.dataset_type + '_' + str(args.thresh_hold) + '.libffm')
    out_valid_path = os.path.join(curr_dir, 'output',
                                 'validffm_' + args.dataset_type + '_' + str(args.thresh_hold) + '.libffm')
    ...
    with open(in_train_path, 'r') as fin, open(out_train_path, 'w') as fout:
        ...
        for line_no, line in enumerate(fin):
            ...
            new_line = [line[0]]
            new_line.extend(map(lambda x: str(x[0]) + str(x[1][0] + int(x[1][1])) + ':1', enumerate(zip(feature_start, line[1:]))))
    ...
    with open(in_valid_path, 'r') as fin, open(out_valid_path, 'w') as fout:
        ...
        for line_no, line in enumerate(fin):
            ...
            new_line = [line[0]]
            new_line.extend(
                map(lambda x: str(x[0]) + str(x[1][0] + int(x[1][1])) + ':1', enumerate(zip(feature_start, line[1:]))))
            fout.write(' \t'.join(new_line) + '\n')
            if (line_no % 400000) == 0:
                logger.info('progress %d', line_no)
        logger.info('progress %d, done ', line_no)
